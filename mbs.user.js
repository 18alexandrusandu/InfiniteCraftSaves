...
.sidebar {
    z-index: 72727 !important;
}
.item.instance.active {
    z-index: 72727 !important;
}
.settings-container {
    display: flex;
...
`
    ...
    const tooltipHandlers = [{
        ...
        enabled: false,
        condition: (e) => wanakana.isJapanese(e.text),
        ...
        description: "hi",
        enabled: false,
        condition: (e) => /^u\+[0-9a-f]{4,5}$/.test(e.text.toLowerCase()),
        handle(element, tooltips) {
            tooltips.push(`🔢 ${space}${String.fromCodePoint(parseInt(element.text.slice(2), 16))}`)
        }
    }]
    ...
    function logCrafts() {
        ...
        unsafeWindow.$nuxt.$root.$children[2].$children[0].$children[0].getCraftResponse = exportFunction((...args) => new window.Promise(async (resolve) => {
            ...
            for (const element of ingredients.map((i) => ({
                    text: i.text,
                    emoji: i.emoji,
            ...
            recentElements = recentElements.slice(0, 10);
            updateRecentElements();
    ...
    const settingsEntries = [{
        ...
        },
        {
            ...
            toggleHandle: () => (settings.pannableBoard = !settings.pannableBoard) && loadSettings(),
            inputs: false
        },
        {
            ...
            toggleHandle: () => (settings.recentElements = !settings.recentElements) && loadSettings(),
            inputs: [{
                label: "Limit:",
    ...
    async function initSettingsModal(elements) {
        ...
        settingsImage.filter = "none!important";
