...
(function() {
    ...
    function spawnElements(locations) {
        ...
        for (const location of locations) {
            location_dict[location[0].toLowerCase()] = new Array(location[1], location[2]);
        }
        console.log(location_dict);
        for (const e of unsafeWindow.$nuxt.$root.$children[2].$children[0].$children[0]._data.elements) {
            if (e.text.toLowerCase() in location_dict) {
                spawnElement(e, location_dict[e.text.toLowerCase()][0], location_dict[e.text.toLowerCase()][1]);
            }
        }
    }
